...
def load_train_val_test_data(
        file_name: str,
        currency: str,
        sizes=(0.85,0,0.15)) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    ...
    train_df, test_df = split_data(df, sizes[0], plot_result=False)

    # Now split again to get validation set
    #valid_df, test_df = split_data(test_df, 0.5, plot_result=False)
...
def calc_benchmarks(df_train, df_test, seasonality=90, plot_result=True, transformed = False):
    """
